...
func (r *endpoint) Apply(ctx context.Context, req ctrl.Request, obj v1alpha1.InnerObject) error {
	...
	expectedContainer := make(map[string]bool)
	for _, container := range golangChaos.Spec.ContainerNames {
		expectedContainer[container] = true
	}
	...
	for podIndex := range pods {
		...
		for _, container := range pod.Status.ContainerStatuses {
			...
			if len(expectedContainer) == 0 || expectedContainer[containerName] {
				g.Go(func() error {
	...
	for _, pod := range pods {
		ps := v1alpha1.PodStatus{
			...
			Message:   fmt.Sprintf(golangSqlQueryErrorActionMsg, golangChaos.Spec.ContainerNames),
		}
